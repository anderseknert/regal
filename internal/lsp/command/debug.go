...
func Debug(ws workspace.Workspace, input *input.Manager, port int, args types.CommandArgs) Command {
	return &debugCommand{workspace: ws, input: input, port: port, args: args}
}

func (d *debugCommand) Run(ctx context.Context) (err error) {
	...
	if d.args.Target == "" || d.args.Query == "" {
		return fmt.Errorf("expected command target and query, got target %q, query %q", d.args.Target, d.args.Query)
	}
	...
	if err = client.Connection().Call(rpcCtx, "regal/startDebugging", d.params(), nil); err != nil {
...
func (d *debugCommand) params() debugParams {
	...
	return debugParams{
		Type: "opa-debug", Request: "launch", Command: "eval", InputPath: inputPath,
		Name: d.args.Query, Query: d.args.Query, EnablePrint: true, StopOnEntry: true, Port: d.port,
	}
}
