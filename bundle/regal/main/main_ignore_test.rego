...
test_ignore_directive_enforced_in_aggregate_rule if {
	report_without_ignore_directives := main.aggregate_report
		with input as {
	...
	report_with_ignore_directives := main.aggregate_report
		with input as {
			"aggregates_internal": {"p.rego": {"imports/unresolved-import": [{}]}},
			"regal": {"file": {"name": "p.rego"}},
...
test_exclude_files_rule_config_with_path_prefix_relative_name if {
	...
	rules_to_run := main._rules_to_run
		with config.rules as rules_config
...
test_force_exclude_file_eval_param if {
	...
	prep := data.regal.prepared.prepare
		with config.rules as {"style": {"prefer-snake-case": {"level": "error"}}}
...
`
	...
	result := main
		with input as object.union(regal.parse_module("p.rego", policy), {"regal": {"operations": ["lint"]}})
